...
exports.rejectBid = exports.acceptBid = exports.getBidsByJob = exports.placeBid = void 0;
...
const getBidsByJob = async (req, res) => {
    try {
        const { jobId } = req.params;
        const bids = await Bid_1.Bid.find({ job: jobId }).populate("freelancer", "name email");
...
exports.getBidsByJob = getBidsByJob;
const acceptBid = async (req, res) => {
    try {
        ...
        if (bid.job.postedBy._id.toString() !== req.user?.id) {
            console.log("Employer ID:", bid.job.postedBy._id.toString());
...
const rejectBid = async (req, res) => {
    try {
        const { bidId } = req.params;
        const bid = await Bid_1.Bid.findById(bidId).populate("job");
        if (!bid)
            return res.status(404).json({ message: "Bid not found" });
